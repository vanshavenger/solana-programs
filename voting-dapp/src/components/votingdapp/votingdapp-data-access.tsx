...
export function useVotingdappProgram() {
  ...
  const initialize = useMutation({
    ...
    mutationFn: (keypair: Keypair) =>
      program.methods
        .initialize()
        .accounts({ votingdapp: keypair.publicKey })
        .signers([keypair])
        .rpc(),
    onSuccess: (signature) => {
...
export function useVotingdappProgramAccount({
  ...
  const closeMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.close().accounts({ votingdapp: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accounts.refetch();
    },
  ...
  const decrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.decrement().accounts({ votingdapp: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
  ...
  const incrementMutation = useMutation({
    ...
    mutationFn: () =>
      program.methods.increment().accounts({ votingdapp: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
  ...
  const setMutation = useMutation({
    ...
    mutationFn: (value: number) =>
      program.methods.set(value).accounts({ votingdapp: account }).rpc(),
    onSuccess: (tx) => {
      transactionToast(tx);
      return accountQuery.refetch();
    },
